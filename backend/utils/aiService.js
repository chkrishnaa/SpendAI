...
export const generateMonthlyInsight = async ({
  ...
  const breakdownText =
    expenseBreakdown.length > 0
      ? expenseBreakdown
          .map((c) => `- ${c.category}: ${currency} ${Number(c.amount || 0).toFixed(2)}`)
          .join("\n")
  ...
  const trendText =
    previousMonths.length > 0
      ? previousMonths
          .map(
            (m) =>
              `- ${m.month}: Income ${currency} ${Number(m.income ?? 0).toFixed(2)}, Expenses ${currency} ${Number(m.expense ?? 0).toFixed(2)}`,
          )
          .join("\n")
...
Currency: ${currency}
Total Income (this month): ${currency} ${Number(totalIncome ?? 0).toFixed(2)}
Total Expenses (this month): ${currency} ${Number(totalExpenses ?? 0).toFixed(2)}
  ...
  const categoryText =
    topCategories.length > 0
      ? topCategories
          .map(
            (c) =>
              `- ${c.category}: ${currency} ${Number(c.amount || 0).toFixed(2)} across ${c.transactionCount} transactions`,
          )
          .join("\n")
...
export const analyzeTransactionList = async ({
  ...
  const prompt = `Analyze these ${transactions.length} transactions and provide a concise, helpful spending insight. Focus on patterns, notable expenses, or anything that stands out. Use specific numbers from the data to support your insight. Tone: friendly and helpful.
